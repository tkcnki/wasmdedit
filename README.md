...
このプロジェクトには Go で書かれた軽量な Web サーバーが付属しています。
...
1. **サーバーを起動します**：
   ...
   ```
2. **ブラウザでアクセスします**：
   `http://localhost:8080` にアクセスしてエディターを起動します。

### WebAssembly (`main.wasm`) のビルド方法
Go 側のパーサーコード（`editor/main.go`）を修正した場合は、以下のコマンドで WebAssembly モジュールを再コンパイルできます：
